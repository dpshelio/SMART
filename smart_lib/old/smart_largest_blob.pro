...
function smart_largest_blob, inmask, indata, flux=flux,nozero=nozero,narr=narr, nosep=nosep

mask=inmask
if keyword_set(flux) gt 0 then data=indata

if keyword_set(nosep) then masksep=mask else masksep=smart_cont_sep(mask, contlevel=.5)
