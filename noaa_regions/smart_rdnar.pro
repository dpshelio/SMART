...
function smart_rdnar, indate0, err=err
print,'SMART_RDNAR: '+strtrim(indate0)
err=''
...
if long(fdate) ge long(strmid(time2file(systim(/utc),/date), 0, 4))*10000+101 then begin
	arch=noaaarch 
	shaundir=0
endif else begin 
	arch=grianarch
...
if strmid(err,0,21) eq 'Could not open socket' then begin & err='' & goto,pingagain & endif
...
;Fill in day and time for NOAA structure
noaastr.time=anytim(file2time(indate),/date,/vms)
noaastr.day=round(anytim(file2time(indate),/date)/(3600.D*24.D),/l64)
